...
from site_selection_util import three_to_one
...
def pLDDT_tag_sites(pdb_file, max_score, window, passes, nsites, min_dist):
	...
	seq = [three_to_one(r.get_resname()) for r in PDB.Selection.unfold_entities(structure, "R")]
	...
	return minima, list(zip(*[seq, smoothed]))
...
if __name__ == "__main__":
	...
	parser.add_argument('--output', action = 'store', type = str, dest = 'SMOOTHED_OUT', 
	...
	parser.add_argument('--window', action = 'store', type = int, dest = "SMOOTHING_WINDOW_SIZE", 
					help = "width of window for smoothing pLDDT (20)", default = 0)
	parser.add_argument('--passes', action = 'store', type = int, dest = "SMOOTHING_PASSES", 
					help = "number of pLDDT smoothing passes (2)", default = 0)
	parser.add_argument('--min_dist', action = 'store', type = int, dest = "SITE_DISTANCE", 
					help = "minimum distance between reported tagging sites (20)", 
					default = 20)
	parser.add_argument('--n_sites', action = 'store', type = int, dest = "N_SITES", 
	...
	if args.SMOOTHED_OUT != None:
		...
		for i in range(len(smoothed_plddt)):
			print("\t".join([str(i+1), smoothed_plddt[i][0], str(smoothed_plddt[i][1])]), file=f_out)
#		print("\n".join(["\t".join([str(x[0]), str(x[1]), str(x[2])]) for x in zip(range(len(smoothed_plddt)), smoothed_plddt)]), file=f_out)
		f_out.close()
